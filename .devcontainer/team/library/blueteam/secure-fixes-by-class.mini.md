...
- Move secrets out of source into environment variables (12-factor), injected at runtime, never via `ENV` or `COPY .env` in the image ([OWASP Docker Security Cheat Sheet](https://cheatsheetseries.owasp.org/cheatsheets/Docker_Security_Cheat_Sheet.html), [Sentry Docker security review](https://github.com/getsentry/skills/blob/main/skills/security-review/infrastructure/docker.md), [Safeguard container image hardening checklist](https://safeguard.sh/resources/blog/container-image-hardening-checklist)).
